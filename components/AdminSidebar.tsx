...
export default function AdminSidebar() {
  ...
  return (
    <>
      <button onClick={() => setCollapsed(!collapsed)} className="lg:hidden fixed top-4 left-4 z-50 p-2 bg-[#1a1a1a] text-white rounded-lg" aria-label="মেনু">
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
      </button>

      <aside className={`fixed top-0 left-0 h-full bg-[#1a1a1a] text-white z-40 transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'} hidden lg:block`}>
        <div className="p-4 border-b border-gray-800">
          <Link href="/admin" className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gold-600 rounded-lg flex items-center justify-center font-bold text-sm">প</div>
            {!collapsed && <span className="font-bold text-lg font-bangla">পাঁচমিশালি</span>}
          </Link>
        </div>
        <nav className="p-3 space-y-1">
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${isActive ? 'bg-gold-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
                ...
                {!collapsed && <span className="text-sm font-bangla">{item.label}</span>}
              </Link>
            );
          })}
        </nav>
      ...
      <aside className={`fixed top-0 left-0 h-full bg-[#1a1a1a] text-white z-40 w-64 transition-transform duration-300 lg:hidden ${collapsed ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-gray-800 flex justify-between items-center">
          <Link href="/admin" className="flex items-center gap-3"><div className="w-8 h-8 bg-gold-600 rounded-lg flex items-center justify-center font-bold text-sm">প</div><span className="font-bold text-lg font-bangla">পাঁচমিশালি</span></Link>
        ...
        <nav className="p-3 space-y-1">
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setCollapsed(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${isActive ? 'bg-gold-600 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
                ...
                <span className="text-sm font-bangla">{item.label}</span>
              </Link>
            );
          })}
        </nav>
